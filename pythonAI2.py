...
def draw_python(python_xy, direction):#, sensor1, sensor2, sensor3):
...
def game():
    ...
    angle_var = atan2(apple_var[0] - head_var[0], apple_var[1] - head_var[1])/pi
...
###############################################################################
            ...
            ###############################################################################3
        plik.write("STATE:"+ str(iterator))
        plik.write("apple = "+ str(apple_var))
        plik.write("angle = "+ str(angle_var))
        plik.write("distance = "+str(distance_var))
        plik.write("reward = "+ str(reward))
        plik.write("is_clear = "+ str(signal1)+" "+ str(signal2)+ " " +  str(signal3))
        plik.write("is_food = "+ str(food1)+" "+ str(food2)+ " " +  str(food3))
        plik.write("is_closer = "+ str(is_closer))
        plik.write("action = "+ str(action))
        plik.write("----------")

        print("STATE:", iterator)
        print("head = ", head_var)
        print("apple = ", apple_var)
        print("angle = ", angle_var)
        print("distance = ", distance_var)
        print("reward = ", reward)
        print("is_clear = ", signal1, signal2, signal3)
        print("is_food = ", food1, food2, food3)
        ...
        brain_input = [signal1, signal2, signal3, food1, food2, food3, is_closer]
...
###############################################################################
        ...
        if python_xy[head]['x'] == 1 or python_xy[head]['x'] == fields_x-2 or python_xy[head]['y'] == 1 or python_xy[head]['y'] == fields_y-2:
            ...
            print("action = ", action)
            print("----------")
            brain_input = [0, 0, 0, food1, food2, food3, is_closer]
            action = brain.update(reward, brain_input)
            score_var.append(brain.score())
        ...
        if signal2 == 0 and signal3 == 0:
            signal1 == 0
            

##########################################################################################################

        #print(signal1, signal2, signal3, food1, food2, food3)
        #print(signal1)
        #print(sensor2)
        #print(sensor3)
        ...
        bg.fill(BG)
        layout()
        draw_python(python_xy, direction)#, sensor1, sensor2, sensor3)
        draw_apple(apple)
